...
namespace caffe {
    ...
    const float NormalizeLayer<Dtype>::EPS = 0.000001f;
    ...
    void NormalizeLayer<Dtype>::Forward_cpu(const vector< Blob<Dtype>* > &bottom,
        ...
        for (int i = 0; i<n; ++i) {
            ...
            caffe_cpu_scale<Dtype>(d, pow(normsqr + EPS, -0.5), bottom_data + i*d, top_data + i*d);
        }
    }
    ...
    void NormalizeLayer<Dtype>::Backward_cpu(const vector< Blob<Dtype>* > &top,
        ...
        for (int i = 0; i<n; ++i) {
            ...
            caffe_cpu_scale(d, Dtype(pow(a + EPS, -0.5)), bottom_diff + i*d, bottom_diff + i*d);
        }
    }
