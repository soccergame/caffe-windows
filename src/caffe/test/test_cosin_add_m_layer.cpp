...
namespace caffe {	
	...
	class CosinAddmLayerTest : public MultiDeviceTest<TypeParam> {
	...
	TYPED_TEST_CASE(CosinAddmLayerTest, TestDtypesAndDevices);

	TYPED_TEST(CosinAddmLayerTest, TestGradient) {
		typedef typename TypeParam::Dtype Dtype;
		LayerParameter layer_param;
        CosinAddmParameter* cosin_param_ptr = layer_param.mutable_cosin_add_m_param();
...
//		rank_param_ptr->set_dist_op(RankHardLossParameter_DistanceOp_Cosine);
        cosin_param_ptr->set_m(0.35);
        cosin_param_ptr->set_transform_test(false);

        CosinAddmLayer<Dtype> layer(layer_param);
